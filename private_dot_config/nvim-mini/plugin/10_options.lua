...
-- Delete empty [No Name] buffers when closing tabs
_G.Config.new_autocmd('TabClosed', nil, function()
  for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
    ...
    then
      ...
      if is_empty and #vim.fn.win_findbuf(bufnr) == 0 then
        pcall(vim.api.nvim_buf_delete, bufnr, { force = true })
      end
    end
  end
end, 'Clean up empty buffers')

-- Chezmoi: auto re-add MiniDeps snapshot after updates
_G.Config.new_autocmd('User', 'MiniDepsUpdateFinished', function()
  local snap_dir = vim.fn.stdpath('config') .. '/deps-snap/'
  for _, f in ipairs(vim.fn.glob(snap_dir .. '*', false, true)) do
    os.execute('chezmoi re-add ' .. f)
  end
end, 'Chezmoi re-add snapshot')
